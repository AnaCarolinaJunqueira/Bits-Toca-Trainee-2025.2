<!DOCTYPE html>
<html lang="pt-BR">

    <head>
        <meta charset="UTF-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <title>Bits toca</title>

        <link rel="stylesheet" href="../../../public/css/listausuarios.css">
        <link rel="stylesheet" href="../../../public/css/modais.css">

        <link rel="stylesheet" href="https://fonts.googleapis.com/css2?family=Roboto:wght@300;400;500;700&display=swap">
        <link rel="stylesheet" href="https://fonts.googleapis.com/css2?family=Nunito:wght@800&display=swap">
        <link rel="stylesheet" href="https://fonts.googleapis.com/css2?family=Inter:wght@400;700&display=swap">
        
        <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.13.1/font/bootstrap-icons.min.css">
    </head>

    <body>
        <main>
            <div class="container">
                <div class="cabeçalho">
                    <div class="logo">
                        <div id="logo">
                            <img src="../../../public/assets/Logo Blog.png" alt="Logo Bits Toca Branca">
                        </div>
                    </div>
                    <div class="titulo">
                        <h1>Tabela de Usuários</h1>
                    </div>
                    <div class="espaço"> </div>
                </div>
                <div class="botoes">
                    <div class="barra-pesquisa">
                        <img src="../../../public/assets/icon_pesquisa.png" alt="icone de pesquisa">
                        <input type="text" id="pesquisa" placeholder="Pesquisar...">
                    </div>
                    <div class="botao-post" onclick="abrirModal('modal-novo-usuario')">
                        <p> NOVO USUÁRIO</p>
                        <i class="bi bi-plus-circle"></i>
                    </div>
                </div>
                <div class="tabela">
                    <div class="tabela-conteudo">
                        <table>
                            <thead>
                                <tr class="header">
                                    <th>
                                        <p>ID</p>
                                    </th>
                                    <th>
                                        <p>NOME</p>
                                    </th>
                                    <th>
                                        <p>EMAIL</p>
                                    </th>
                                  
                                    <th class="action">
                                        <p>AÇÕES</p>
                                    </th>
                                </tr>
                            </thead>
                            <tbody>
                                <?php foreach($users as $user): ?>
                                <tr>
                                    <td class="items">
                                        <p><?= $user->ID ?></p>
                                    </td>
                                    <td class="items">
                                        <p><?= $user->NOME ?></p>
                                    </td>
                                    <td class="items">
                                        <p><?= $user->EMAIL ?></p>
                                    </td>
                                   
                                    <td class="açoes">
                                        <div>
                                            <div class="button-content">
                                                <button class="btn-modal" onclick="abrirModal('modal-visualizar-usuario')">
                                                    <i class="bi bi-eye"></i>
                                                </button>
                                            </div>
                                            <div class="button-content">
                                                <button class="btn-modal" onclick="abrirModal('modal-editar-usuario')">
                                                    <i class="bi bi-pencil"></i>
                                                </button>
                                            </div>
                                            <div class="button-content">
                                                <button class="btn-modal" onclick="abrirModal('modal-deletar-usuario')">
                                                    <i class="bi bi-trash"></i>
                                                </button>
                                            </div>
                                        </div>
                                    </td>
                                </tr>

                                <?php endforeach; ?>
                                
                            </tbody>
                        </table>
                    </div>
                    <div class="tabela-paginacao">
                        <table>
                            <tfoot>
                                <tr>
                                    <td colspan="5">
                                        <div class="foot">
                                            <div class="button-content">
                                                <button class="botao_foot">
                                                    <i class="bi bi-caret-left-fill"></i>
                                                </button>
                                            </div>
                                            <div>
                                                <p>1</p>
                                            </div>
                                            <div class="button-content">
                                                <button class="botao_foot">
                                                    <i class="bi bi-caret-right-fill"></i>
                                                </button>
                                            </div>
                                        </div>
                                    </td>
                                </tr>
                            </tfoot>
                        </table>
                    </div>
                    <img src="../../../public/assets/7.png" class="figurinha" id="fig1" alt="">
                    <img src="../../../public/assets/26.png" class="figurinha" id="fig2" alt="">
                    <img src="../../../public/assets/38.png" class="figurinha" id="fig3" alt="">
                </div>
            </div>
        </main>

    <?php require 'app/views/admin/modals/modal-novo-usuario.html'; ?>
    <?php require 'app/views/admin/modals/modal-editar-usuario.html'; ?>
    <?php require 'app/views/admin/modals/modal-deletar-usuario.html'; ?>
    <?php require 'app/views/admin/modals/modal-visualizar-usuario.html'; ?>
    <?php require 'app/views/admin/modals/modal-goto-page.html'; ?>
    
    <script src="/public/js/listusuario.js"></script>
    <script src="/public/js/modals.js"></script>


    </body>
</html>